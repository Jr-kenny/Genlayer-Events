...
const Timeline = ({ filter }: TimelineProps) => {
  ...
  const { events, loading, error } = useGoogleSheets();
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="font-body text-muted-foreground text-xs md:text-sm uppercase tracking-widest">
              {loading ? 'Loading...' : `${filteredEvents.length} ${filter} ${filteredEvents.length === 1 ? 'event' : 'events'}`}
            </p>
          </motion.div>
        </AnimatePresence>
        ...
        {error && (
          <motion.div 
            ...
            className="text-center py-16"
          >
            <p className="font-body text-destructive">Error loading events: {error}</p>
          </motion.div>
        )}
